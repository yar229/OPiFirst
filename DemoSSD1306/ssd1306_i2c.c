...
//
//int buffer1[SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8] =
//{
//	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
//	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
//	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
{
	//memset(&buffer1, 0, SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8);
...
void ssd1306_display(void)
{
	...
	// I2C
	int size = (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8);
	int i;
	//for (i = 0; i < size; i++) 
	//{
	//	//if (buffer[i] != buffer1[i])
	//		wiringPiI2CWriteReg8(i2cd, 0x40, reverse_1byte(buffer[i]));
	//}
	...
	//memcpy(&buffer, &buffer1, SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8);
...
void ssd1306_copyBuffer(int *inputBuffer, int size)
{
	...
	//else
	//{
	//	copy_reverse(&buffer, inputBuffer, size);
	//}
}

//void copy_reverse(unsigned int *restrict dest, unsigned int const *restrict src, unsigned int n)
//{
//	unsigned int i, x;
//	for (i = 0; i < n / 2; ++i) {
//		x = src[i];
...
//void ssd1306_drawBitmap(int16_t x, int16_t y, const int bitmap[], int16_t w, int16_t h, uint8_t foreColor, uint8_t backColor)
//{
...
//			//if (byte & 0x80)
//			uint8_t color = byte & 0x80 ? foreColor : backColor;
	...
	for (j = 0; j < h; j++) 
	{
		for (i = 0; i < w; i++) 
		{
			if (pgm_read_byte(bitmap + j * byteWidth + i / 8) & (128 >> (i & 7))) 
				ssd1306_drawPixel(x + i, y + j, backColor);
			else 
				ssd1306_drawPixel(x + i, y + j, foreColor);
		}
	}
}